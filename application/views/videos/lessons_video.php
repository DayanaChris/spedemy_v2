


  <?php
    $this->load->view('templates/temp_alphabets');
    $this->load->view('templates/nav');  ?>




     <body class="run-animation category_menu_bg" >



                   <div class="container-fluid " style=" margin-top: 0%;margin-bottom: 0%; padding-top: 70px;" id="shad" >
                       <div class="card run-animation" style="background: #CBE1FB; ">
                          <div class="card-header" style=" background:#FF1E9D"> <h2> <b> LESSON VIDEOS </b></h2></div>
                          <a href="<?php echo base_url(); ?>videos_menu "> <img class="btn zoom  container float-right card-img-top img-fluid   " style="width:100px; margin-bottom:0px" src="<?php echo site_url(); ?>assets/images/SPEDEMY/video_button.png"   /></a>

                          <!-- <a href="<?php echo base_url(); ?>videos_menu"> <img class="zoom  container float-right card-img-top img-fluid   " style=" width: 10%;
                              margin-top: 0px;
                              border: 0;
                              background: transparent; " src="<?php echo site_url(); ?>assets/images/SPEDEMY/Videos/video_button.png"   /></a> -->

                               <div     >
                                      <br>


                                            <div>

                                                              <div class="container "   >
                                                                <!-- style="padding-bottom: 150px" -->
                                                                <div class="col-sm-4   " style="padding-top:50px" >
                                                                  <video width="400" controls>
                                                                      <source src="<?php echo site_url(); ?>assets/images/SPEDEMY/Videos/lesson/Alphabets.mp4" type="video/mp4">
                                                                    </video>
                                                                </div>

                                                                <div class="col-sm-4   " style="padding-top:50px" >
                                                                  <video width="400" controls>
                                                                      <source src="<?php echo site_url(); ?>assets/images/SPEDEMY/Videos/lesson/Colors.mp4" type="video/mp4">
                                                                    </video>
                                                                </div>
                                                                <div class="col-sm-4   " style="padding-top:50px" >
                                                                  <video width="400" controls>
                                                                      <source src="<?php echo site_url(); ?>assets/images/SPEDEMY/Videos/lesson/FacialExpression.mp4" type="video/mp4">
                                                                    </video>
                                                                </div>
                                                                <div class="col-sm-4   " style="padding-top:50px" >
                                                                  <video width="400" controls>
                                                                      <source src="<?php echo site_url(); ?>assets/images/SPEDEMY/Videos/lesson/Numbers.mp4" type="video/mp4">
                                                                    </video>
                                                                </div>
                                                                <div class="col-sm-4   " style="padding-top:50px" >
                                                                  <video width="400" controls>
                                                                      <source src="<?php echo site_url(); ?>assets/images/SPEDEMY/Videos/lesson/Shapes.mp4" type="video/mp4">
                                                                    </video>
                                                                </div>
                                                                <div class="col-sm-4   " style="padding-top:50px" >
                                                                  <video width="400" controls>
                                                                      <source src="<?php echo site_url(); ?>assets/images/SPEDEMY/Videos/lesson/Action Words.mp4" type="video/mp4">
                                                                    </video>
                                                                </div>

                                                              </div>

                                            </div>
                                            <br><br> <br>
                                          </div>

                                          <div class="item " >
                                            <div>




                                            </div>
                                            <br><br> <br>
                                          </div>
                                        </div>





                                      </div>

                                    </div> <br>

                           </div>
                   </div>





                   <script src="https://code.jquery.com/jquery-3.2.1.slim.min.js" integrity="sha384-KJ3o2DKtIkvYIK3UENzmM7KCkRr/rE9/Qpg6aAZGJwFDMVNA/GpGFF93hXpG5KkN" crossorigin="anonymous"></script>
                   <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.12.9/umd/popper.min.js" integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q" crossorigin="anonymous"></script>
                   <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script>


                   <script src="<?php echo base_url(); ?>assets/js/app.js"></script>

                   <script src="<?php echo base_url(); ?>assets/js/animate.js"></script>
     </body>
</html>
